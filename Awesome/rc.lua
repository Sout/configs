...
naughty.config.defaults.font = "Monaco 8"
...
mailfolders =   {
'/home/jhon/.thunderbird/xut8mymi.default/ImapMail/mail.thinkrf.com/INBOX',
}
mailhoover.addToWidget(mailicon, mailfolders, 'Thinkrf')
...
--local out = (io.popen("python /home/jhon/Git/weather/test.py")):read("*a")
--naughty.notify({title="Weather",text=out}) 
--end )
--mytimer:start()
-- {{{ Shutdown
...
--orglendar.files = { "/home/jhon/Documents/work.org"} 
...
for s = 1, screen.count() do
  ...
  if s == 2 then right_wibox:add(wibox.widget.systray()) 
  ...
  -- Graphbox
  mygraphbox[s] = awful.wibox({ position = "bottom", height = 30, screen = s })
  ...
  left_graphbox:add(space)
  left_graphbox:add(txwidgett)
  left_graphbox:add(txgraph)
  left_graphbox:add(txwidget)
  left_graphbox:add(tab)
  left_graphbox:add(rxwidgett)
...
globalkeys = awful.util.table.join(
  ...
  -- Standard program
  awful.key({ modkey, }, "Return", function() awful.util.spawn(terminal,false,mouse.screen) end),
...
awful.rules.rules = {
  ...
  { rule = { class = "Thunderbird", instance = "Mail" },
    properties = { floating = true, above = true, tag = tags[2][1]} },
